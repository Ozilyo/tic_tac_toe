...
module TicTacToe
	describe Board do
	  ...
	  context "#initialize" do
	  	...
	  	it 'initalizes with 3 raws by default' do
	  		board = Board.new
	  		expect((board.grid).size).to eq 3
	  	end

	  	it 'creates 3 things on each row by default' do
	  		board = Board.new
	  ...
	  context '#grid' do
	  	it 'returns the grid value' do
	  		board = Board.new({grid: "grid value"})
	  		expect(board.grid).to eq "grid value"
	  	end
	  end
	  ...
	  context '#get_cell' do
	  	it 'returns the cell value based on (x,y) coordinates' do
	  		...
	  		expect(board.get_cell(1,2)).to eq 'Ali'
	  	end
	  end

	  context '#set_cell' do
	  	it 'sets the cell object value on the specified (x,y) coordinates' do
	  		Cat = Struct.new(:value)
	  		grid = [['','','Cat.new("kitty")'],['','',''],['','Ali','']]
	  		board = Board.new(grid: grid)
	  		expect(board.set_cell(2,0, "kitten")).to eq 'kitten'
	  	end
	  end
	  ...
	  # 	it 'returns :winner if winner? is false and draw? is false' do
	  # 		board = Board.new
	  # 		board.stub(:winner?) { false }
